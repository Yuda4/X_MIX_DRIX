...
const Coordinate XYPlayer::play(const Board& board) { //rows
	for (uint x=0; x<board.size(); ++x) {
		for (uint y=0; y<board.size(); ++y) {
			Coordinate c{static_cast<int>(x),static_cast<int>(y)};
			if (board[c]=='.') {
				return c;
			}
		}
...
const Coordinate YXPlayer::play(const Board& board) { //columns
	for (uint y=0; y<board.size(); ++y) {
		for (uint x=0; x<board.size(); ++x) {
			Coordinate c{static_cast<int>(x),static_cast<int>(y)};
			if (board[c]=='.') {
